...
function getCacheKey(msg) {
    return msg.trim().toLowerCase().replace(/[^\u0600-\u06FFa-zA-Z0-9\s]/g, '');
}
...
// ===== استدعاء Gemini API مع إدارة ذكية =====
...
async function processMessage(userMessage) {
    ...
    try {
        ...
        if (e.message === 'RATE_LIMIT') {
            return 'الخدمة مزدحمة حالياً. يرجى الانتظار 30 ثانية ثم المحاولة مرة أخرى.\n\nنصيحة: لا ترسل أكثر من سؤال كل 10 ثوانٍ.';
        }
...
function saveSettings() {
    ...
    addMessage('تم حفظ الإعدادات!', false);
}
